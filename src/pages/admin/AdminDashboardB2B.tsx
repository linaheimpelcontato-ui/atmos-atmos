  ...
  return (
    <div className="p-4 md:p-8 space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-foreground">Dashboard B2B</h1>
          <p className="text-muted-foreground mt-1">Análise completa do pipeline de parcerias e imersões</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-4">
        {[
          { title: "Total Prospects", value: kpis.total, icon: Building2, color: "text-primary", bg: "bg-primary/10" },
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <Card className="shadow-sm">
          <CardHeader className="pb-2">
            ...
            <CardDescription>De onde vêm os leads B2B</CardDescription>
          </CardHeader>
          <CardContent>
            <ResponsiveContainer width="100%" height={280}>
              <PieChart>
        ...
        <Card className="shadow-sm">
          ...
          <CardContent>
            <Table>
              <TableHeader>
                <TableRow>
                  <SmartTableHead label="Origem" sortKey="name" filterState={sourceTableFilter} data={sourceData} className="text-xs" />
                  <SmartTableHead label="Prospects" sortKey="prospects" filterState={sourceTableFilter} data={sourceData} className="text-xs text-center" />
                  <SmartTableHead label="Propostas" sortKey="proposals" filterState={sourceTableFilter} data={sourceData} className="text-xs text-center" />
                  <SmartTableHead label="Aceitas" sortKey="accepted" filterState={sourceTableFilter} data={sourceData} className="text-xs text-center" />
                  <SmartTableHead label="Conv." sortKey="convRate" filterState={sourceTableFilter} data={sourceData} className="text-xs text-center" />
                  <SmartTableHead label="Receita" sortKey="revenue" filterState={sourceTableFilter} data={sourceData} className="text-xs text-right" />
                </TableRow>
              </TableHeader>
              <TableBody>
                {sourceTableFilter.applyFilters(sourceData).length === 0 ? (
      ...
      {/* Row 3: Top Partners + Seller Ranking */}
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <Card className="shadow-sm">
          <CardHeader className="pb-2">
            <CardTitle className="text-base flex items-center gap-2">
              <Award className="h-4 w-4 text-primary" /> Top 10 Parceiros
            </CardTitle>
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-muted-foreground">
                          {c.company && `${c.company} · `}{c.country && `${c.country} · `}{c.count} proposta{c.count > 1 ? "s" : ""}
                        </p>
                      </div>
                    </div>
          ...
          <CardContent>
            {sellerRanking.length === 0 ? (
              ...
              <Table>
                <TableHeader>
                  <TableRow>
                    <SmartTableHead label="Vendedor" sortKey="name" filterState={sellerTableFilter} data={sellerRanking} className="text-xs" />
                    <SmartTableHead label="Prosp." sortKey="prospects" filterState={sellerTableFilter} data={sellerRanking} className="text-xs text-center" />
                    <SmartTableHead label="Prop." sortKey="proposals" filterState={sellerTableFilter} data={sellerRanking} className="text-xs text-center" />
                    <SmartTableHead label="Conv." sortKey="convRate" filterState={sellerTableFilter} data={sellerRanking} className="text-xs text-center" />
                    <SmartTableHead label="Receita" sortKey="revenue" filterState={sellerTableFilter} data={sellerRanking} className="text-xs text-right" />
                  </TableRow>
                </TableHeader>
                <TableBody>
            ...
            {upcomingFollowups.length === 0 ? (
              ...
              <div className="space-y-2">
                {upcomingFollowups.map((p: Prospect) => (
                  <Link key={p.id as string} to="/admin/b2b/prospects" className="flex items-center justify-between p-3 rounded-lg bg-muted hover:bg-muted/80 transition-colors">
                    <div className="flex items-center gap-3">
                      <Building2 className="h-4 w-4 text-primary" />
                      <div>
                        <p className="text-sm font-medium">{p.name as string}</p>
                        <p className="text-xs text-muted-foreground">{p.company_type as string} · {p.country as string}</p>
                      </div>
                    </div>
        ...
        <Card className="shadow-sm">
          ...
          <CardContent>
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
              {[
                { to: "/admin/b2b/prospects", label: "Prospects B2B", desc: "Lista completa", icon: Building2 },
                { to: "/admin/b2b/pipeline", label: "Pipeline B2B", desc: "Kanban visual", icon: TrendingUp },
                { to: "/admin/b2b/propostas", label: "Propostas B2B", desc: "Orçamentos", icon: Star },
              ].map(item => (
