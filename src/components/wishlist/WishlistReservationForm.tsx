...
import { type WishlistItemType } from "@/contexts/WishlistContext";

export interface WishlistReservationAnswers {
  profileType: string;
  status: string;
...
const emptyAnswers = (user: any): WishlistReservationAnswers => ({
  profileType: "",
  status: "",
  startDate: undefined,
  endDate: undefined,
...
interface Props {
  items: { id: string; type: WishlistItemType; name: string; details?: string }[];
  language: Language;
  onClose: () => void;
}
...
function buildWhatsAppMessage(
  ...
  answers: WishlistReservationAnswers,
  lang: Language
) {
  const greeting = txt(
    lang,
    "Olá! Gostaria de solicitar um orçamento personalizado para minhas preferências. Seguem minhas informações e interesses:",
    "Hello! I'd like to request a custom quote for my preferences. Here are my details and interests:",
    "¡Hola! Me gustaría solicitar un presupuesto personalizado para mis preferencias. Aquí mis datos e intereses:"
  );
...
interface QuestionDef {
  id: keyof WishlistReservationAnswers;
  ...
  conditional?: (answers: WishlistReservationAnswers) => boolean;
...
export default function WishlistReservationForm({ items, language, onClose }: Props) {
  const { user } = useAuth();
  const [answers, setAnswers] = useState<WishlistReservationAnswers>(emptyAnswers(user));
  ...
  const handleOnboardingConfirm = async () => {
    ...
    trackWhatsAppClick("quote_wishlist_direct");
        ...
        {!showOnboarding ? (
          ...
          >
            ...
            <div className="flex items-center justify-between">
              ...
              <span className="text-[10px] uppercase tracking-widest text-[#c4a97d] font-bold">
                {txt(language, "SUAS PREFERÊNCIAS", "YOUR PREFERENCES", "TUS PREFERENCIAS")}
              </span>
            </div>
              ...
              >
                {isLast ? (
                  <>
                    ...
                    {txt(language, "Solicitar Orçamento", "Request Quote", "Solicitar Presupuesto")}
                  </>
                ) : (
                  <>
          ...
          >
            ...
            <h2 className="text-2xl font-display uppercase tracking-tight text-[#2e2019] font-outfit">
              {txt(language, "Como funciona nosso roteiro?", "How does our itinerary design work?", "¿Cómo funciona el diseño de nuestro itinerario?")}
            </h2>
