...
export function AtmosCard({ title, subtitle, headerAction, children, className, ...props }: AtmosCardProps) {
  return (
    <div 
      className={cn(
        "bg-white dark:bg-admin-surface border border-admin-border rounded-2xl shadow-[0_4px_24px_-4px_rgba(0,0,0,0.04)] overflow-hidden flex flex-col transition-all",
        className
      )}
      {...props}
    >
      {(title || subtitle || headerAction) && (
        <div className="px-6 py-5 border-b border-admin-border/50 flex items-center justify-between bg-white/50 backdrop-blur-sm">
          <div>
            {title && <h3 className="text-lg font-bold tracking-tight text-admin-primary dark:text-white leading-none">{title}</h3>}
