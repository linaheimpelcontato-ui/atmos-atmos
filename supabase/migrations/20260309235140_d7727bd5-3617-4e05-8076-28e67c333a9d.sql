...
ON public.proposal_days FOR ALL TO authenticated
USING (has_role(auth.uid(), 'admin'::app_role))
WITH CHECK (has_role(auth.uid(), 'admin'::app_role));

CREATE POLICY "Customer view published proposal_days"
ON public.proposal_days FOR SELECT TO authenticated
USING (
  EXISTS (
...
ON public.proposal_days FOR SELECT TO anon, authenticated
USING (
  EXISTS (
...
ON public.proposal_day_items FOR ALL TO authenticated
USING (has_role(auth.uid(), 'admin'::app_role))
WITH CHECK (has_role(auth.uid(), 'admin'::app_role));

CREATE POLICY "Customer view published proposal_day_items"
ON public.proposal_day_items FOR SELECT TO authenticated
USING (
  EXISTS (
    SELECT 1 FROM public.proposals pr
    JOIN public.prospects p ON p.id = pr.prospect_id
...
ON public.proposal_day_items FOR SELECT TO anon
USING (
  EXISTS (
    SELECT 1 FROM public.proposals
