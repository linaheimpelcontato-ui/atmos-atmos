...
BEGIN
 -- RPC INSERTs use jsonb_populate_record: explicitly supply every NOT NULL payload field.
 -- Proposals: title/status/segment/subtotal/discount_percent/discount_fixed/tax_percent/total/language.
 -- Items: day_number/item_index/category/value/quantity/cost_price/commission_percent; IDs supplied/generated by RPC.
 r:=public.save_proposal_bundle(NULL,'{"title":"Identity fixture","status":"draft","segment":"b2c","subtotal":0,"discount_percent":0,"discount_fixed":0,"tax_percent":0,"total":0,"language":"pt"}',
 jsonb_build_array(jsonb_build_object('id',a,'day_number',1,'item_index',0,'category','Serviços','item_name','A','value',0,'quantity',1,'cost_price',20,'commission_percent',0),
 jsonb_build_object('id',b,'day_number',1,'item_index',1,'category','Serviços','item_name','B','value',0,'quantity',1,'cost_price',40,'commission_percent',0)), '[]','[]','[]','[]');
 p:=(r->>'id')::uuid;
...
-- Direct INSERTs omit defaulted columns; unlike RPC jsonb_populate_record, these do use column defaults.
